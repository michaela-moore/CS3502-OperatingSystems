                ...
                if (firstResourceLocked) { // first is locked
                    ...
                    if (secondResourceLocked) { // second is locked
                        ...
                        if (amount < this.Balance) {
                            ...
                            Console.WriteLine($"\n--> Not enough funds to transfer {amount} | Balance: {RoundAmount(Balance)}");
                        }
                    } else { // second is not locked
                        Console.WriteLine($"\n** Could not get second lock on 'to' account {to.GetId()}");
                    }
                } else { // first is not locked
                    Console.WriteLine($"\n** Could not get first lock on 'from' account {Id}");
                }
            } finally {
                // Release the locks
